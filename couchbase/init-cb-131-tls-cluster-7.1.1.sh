...
couchbase-cli server-add \
                ...
                --no-ssl-verify \
                --password password \
                --server-add https://10.229.214.132:18091 \
...
couchbase-cli server-add \
                ...
                --no-ssl-verify \
                --password password \
                --server-add https://10.229.214.133:18091 \
                --server-add-username Administrator \
                --server-add-password password \
                --services data
sleep 3s
...
cp /app/config/ca.pem /opt/couchbase/var/lib/couchbase/inbox/CA
couchbase-cli ssl-manage -c 10.229.214.131 -u Administrator -p password --cluster-ca-load 
sleep 3s
# update node cert.
